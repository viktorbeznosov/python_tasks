...
def split_markdown_text_by_path(folder, path):
    text_file = os.path.join(folder, path)
    with open(text_file, 'r', encoding='utf-8') as txt_file:
        text_md = txt_file.read()
    return split_markdown_text(text_md)    
...
url= 'https://vkvideo.ru/video-226932274_456239017'
folder = './content/'
audio_file_name = 'Python в вопросах и ответах'
...
audio_path = os.path.join(folder, "Python в вопросах и ответах.m4a")
file_title = 'Python в вопросах и ответах'
...
# @titleФормируем разделы для текста
...
# Текстовый файл с диска
text_file = os.path.join(folder, "Python в вопросах и ответах.txt")
with open(text_file, 'r', encoding='utf-8') as txt_file:
    text = txt_file.read()
...
chunks_md_splits = split_markdown_text_by_path(folder, "processed_md_text.txt")
...
# # Получаем список документов, разбитых по заголовкам
chunks_md_splits = split_markdown_text_by_path(folder, "short_tutorial.txt")

# # Получаем индексную базу данных
db_vector = get_db_vektor(folder, 'db_vector_python', chunks_md_splits)

while True:
    system = """Ты - преподаватель, эксперт по Python. Твоя задача - ответить
    ...
    answer = format_text(generate_db_answer(query, db_vector, verbose=False))
    print(f'\nОтвет косультанта: \n\n{answer}')
